...
func main() {
	...
	for i := 1; i <= 1; i++ {
		...
		// Парсинг статистики
		var reqsPerSec, latency, latency50, latency75, latency90, latency95, latency99, http1xx, http2xx, http3xx, http4xx, http5xx, otherHTTP, throughput string

		for _, line := range strings.Split(output, "\n") {
			if strings.Contains(line, "Reqs/sec") {
	...
	// Вывод статистики
	fmt.Println("\nStatistics")
	// Здесь мы не используем avg, stdev и max, так как они не были определены в предыдущем коде
}
